		...
		if((((un_p_info)->extra).constructor).con_num == 0)
		{
			...
			if(un_f_info.type == 0)
			{
			...
			else if(un_f_info.type == 4)
				{
					...
					unroll_pap(un_f);
					ref var_63;
					void* var_66 = (void*)(((*((((un_f_info).extra).pap_info).info_ptr)).extra).function).slow_entry_point;
					__asm__ volatile (
						"movq %0, %%rdi;\n\t"
						"movq %1, %%rsi;\n\t"
						"movq %%rbp, %%rsp;\n\t"
...
ref var_46(ref thunk_ref)
{
	...
	if(ma_f_info.type == 0)
	{
	...
	else if(ma_f_info.type == 4)
		{
			...
			unroll_pap(ma_f);
			ref var_78;
			void* var_81 = (void*)(((*((((ma_f_info).extra).pap_info).info_ptr)).extra).function).slow_entry_point;
			__asm__ volatile (
				"movq %0, %%rdi;\n\t"
				"movq %1, %%rsi;\n\t"
				"movq %%rbp, %%rsp;\n\t"
...
ref main_function()
{
	...
	while(stack_pointer < stack_top) {
		var_92 = case_continuation(updateContinuation(var_92));
	}
	return var_92;
}
